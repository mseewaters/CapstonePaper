...
data.raw <- read.csv("NIBRS_datafinal_1999.csv", header=FALSE, na.strings=c("NA"))
...
ucr.list <- c(100,111,112,113,114,131,132)
weapon.list <- c(110,111,120,121,130,131,140,141,150,151,200,300,350,500,600,650,700,850)

data.new$targ1 <- ifelse((data.new$victim.injury>=3 | data.new$V20062 %in% ucr.list | data.new$V20171 %in% weapon.list),1,0)

table(data.new$targ1)
...
#change names to real names
names.data <- c('SEGMENT', 'STATE', 'ORI', 'INCNUM', 'INCDATE', 'ori.nibrs.date', 'city.name', 
                'population.group', 'division', 'region', 'current.population', 'UCR.County', 'FIPS.County', 'report.date', 
                'incident.hour', 'UCR.code1', 'UCR.code2', 'UCR.code3', 'completed1', 'completed2', 'completed3', 
                'location', 'weapon1', 'weapon2', 'homicide1', 'homicide2', 'arrest.date1', 'arrest.date2', 
                'type.arrest1', 'type.arrest2', 'arrest.code1', 'arrest.code2', 'agemax', 'VNumVG','VNumOG', 'type','primary.incident',
...
names.data2 <- c('SEGMENT', 'STATE', 'ORI', 'INCNUM', 'INCDATE', 'B1006', 'B1007', 
                'B1009', 'B1010', 'B1011', 'B2005', 'B2006', 'B3024', 'V1006', 
                'V1007', 'V20061', 'V20062', 'V20063', 'V20071', 'V20072', 'V20073', 
...
x <- as.data.frame(cbind(data.empty, names.data2, names.data))[order(-data.empty),]
x <- x[which(x$data.empty != 0),]
x


# remove fields with more then 50% empty, retain UCR code 2 as that was needed for creating target variable

data.t <- data[,-which(names(data) %in% x[c(1:8,10:13),2])]
...
data.t$victimtenderage.group <- cut(data.t$victim.age, 
                              breaks=c(-0.5,12,Inf), 
                              labels=c('Yes','No'))
...
write.csv(data.t, file="abduction_cleaned_coded_1999.csv")


# Break to clear memory ---------------------------------------------------

data.t <- read.csv("abduction_cleaned_coded_opt3.csv")
data.t <- read.csv("abduction_cleaned_coded_1999.csv")
...
fullnames.data <- c('state','originating.agency', 'incident.number', 'incident.date', 'date.agency.went.NIBRS',
                    ...
                    'UCR.county', 'FIPS.county', 'report.date', 'incident.hour', 'UCR.code', 'UCR.code2',
                  ...
                  'offender.age', 'offender.sex', 'offender.race','target.harm','target.arrest','victimage.group', 'victimtender.age',
...
statefipsmap <- read.csv("D:/0 Stern MSBA/0.2 Abduction/abduction2/statefipsmap.csv")
data.merge <- merge(data.merge, statefipsmap, by='state.name',all.x=TRUE)
